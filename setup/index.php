...
$accounts_conf = $_POST['acc'] ?? parse_ini_file(ACC_STORAGE_INI_FILE);
$errs = [];
  ...
  <body>
    ...
    <form method="post">
      ...
      <p title="The reference implementation uses these two microservices (with as yet undocumented apis)">
        Account store <input name = "account_store_url" value = "<?php print $node_conf['account_store_url']; ?>" placeholder = "https://accounts.mydomain.com">
      <br />Business logic <input name = "blogic_service_url" value = "<?php print $node_conf['blogic_service_url']; ?>" placeholder = "https://blogic.mydomain.com">  (optional)
      </p>

      <h2>Database settings</h2>
      <p>Db name <input name = "db[name]" value = "<?php print $node_conf['db']['name']; ?>">
        <br />Db user <input name = "db[user]" value = "<?php print $node_conf['db']['user']; ?>">
        <br /><span title="Password is not required for the moment">Db pass <input name = "db[pass]" value = "<?php print $node_conf['db']['pass']; ?>"></span>
      </p>


      <h2>Default values for new accounts</h2>
      <p>Max account limit: <input name="acc[default_max]" type="number" min="1" max="1000000" size="3" value="<?php print $accounts_conf['default_max']; ?>" />
      <br />Min account limit: <input name="acc[default_min]" type="number" max="0" min="-1000000" size="3" value="<?php print $accounts_conf['default_min']; ?>" />
      <br />Active <input name="acc[default_status]" type="checkbox" value = "1"<?php if ($accounts_conf['default_status']) print ' checked'; ?> />
